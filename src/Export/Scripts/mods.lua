...
-- Synthesis implicits no longer expose spawn weights in Mods.dat, so export the
-- authoritative item class restrictions from ItemSynthesisMods.dat separately.
local synthesisModItemClasses = { }
for entry in dat("ItemSynthesisMods"):Rows() do
	for _, mod in ipairs(entry.ImplicitMod or { }) do
		local itemClasses = synthesisModItemClasses[mod.Id]
		if not itemClasses then
			itemClasses = { }
			synthesisModItemClasses[mod.Id] = itemClasses
		end
		for _, itemClass in ipairs(entry.ItemClasses or { }) do
			itemClasses[itemClass.Id] = true
		end
	end
end

local synthesisModItemClassesOut = io.open("../Data/SynthesisModItemClasses.lua", "w")
synthesisModItemClassesOut:write('-- This file is automatically generated, do not edit!\n')
synthesisModItemClassesOut:write('-- Item data (c) Grinding Gear Games\n\nreturn {\n')
for modId, itemClasses in pairsSortByKey(synthesisModItemClasses) do
	synthesisModItemClassesOut:write('\t["', modId, '"] = { ')
	for itemClassId in pairsSortByKey(itemClasses) do
		synthesisModItemClassesOut:write('["', itemClassId, '"] = true, ')
	end
	synthesisModItemClassesOut:write('},\n')
end
synthesisModItemClassesOut:write('}\n')
synthesisModItemClassesOut:close()
